...
def click_csv_button():
    clear()
    stop()
    display_frame.configure(text="Display CSV",foreground="white",background="#030335", font=('ariel', 30))
    display_csv()

def click_register_button():
    clear()
    stop()
    display_frame.configure(text="Register Finger",foreground="white",background="#030335", font=('ariel', 30))
    text_widget.pack()
...
def click_verify_button():
    clear()
    stop()
    display_frame.configure(text="Verify Finger",foreground="white",background="#030335", font=('ariel', 30))
    text_widget.pack()
    text_widget.delete("1.0", tk.END)  # Clear the text widget
...
def click_attendance_button():
    clear()
    stop()
    display_frame.configure(text="Attendance",foreground="white",background="#030335", font=('ariel', 30))
    text_widget.pack()
    text_widget.delete("1.0", tk.END)  # Clear the text widget
...
def click_delete_button():
    clear()
    stop()
    display_frame.configure(text="Delete Finger",foreground="white",background="#030335", font=('Helvetica', 30))
    text_widget.pack()
...
def click_email_button():
    clear()
    stop()
    display_frame.configure(text="Send Email",foreground="white",background="#030335", font=('Helvetica', 30))
    text_widget.pack()
    text_widget.delete("1.0", tk.END)
...
def save_fingerprint_image(image_array, filename):
    """Save fingerprint image to the specified directory"""
    image_path = os.path.join(
        "D:\\Wai_yan_folder\\programming\\fingerprint_recognition\\finger_images", filename)
    plt.imsave(image_path, image_array, cmap='gray')

def get_fingerprint_photo():
    """Get and save fingerprint image"""
    ...
    # Generate a unique filename for each fingerprint image (you can customize this part)
    fingerprint_id = len(os.listdir("D:\\Wai_yan_folder\\programming\\fingerprint_recognition\\finger_images"))
    filename = f"{user_input}_{fingerprint_id}.png"
    
    # Save the fingerprint image
    save_fingerprint_image(imgArray, filename)
...
display_frame = ttk.Label(left_frame, text="", font=("Helvetica", 20), background="#030335", foreground="#2196F3")
...
# MIIt logo
miit = Image.open("D:\\Wai_yan_folder\\programming\\fingerprint_recognition\\fingerprint_authentication_system\\icon\\miit_logo.png")  # Replace with the actual path to your image
...
text = tk.Label(left_frame, text="Fingerprint Authentication System", font=("Helvetica", 30), background="#030335", fg="white")
text.pack()

# Create a text widget and buttons for displaying the output
text_widget = tk.Text(left_frame, font=("Helvetica", 14), wrap=tk.WORD,background="#030335",fg="white")
# Create buttons with custom style
start_button = ttk.Button(left_frame, text="Start", command=start, style="TButton",width=8)
stop_button = ttk.Button(left_frame, text="Stop", command=stop, style="TButton",width=8)
verify_button = ttk.Button(left_frame, text="Check", command=verify, style="TButton",width=8)
confirm_button = ttk.Button(left_frame, text="Confirm", command=register, style="TButton",width=8)
delete_button = ttk.Button(left_frame, text="Delete", command=delete, style="TButton",width=8)
default_button = ttk.Button(left_frame, text="Default", command=default, style="TButton",width=8)
send_button = ttk.Button(left_frame, text="Send", command=secure, style="TButton",width=8)
...
image = Image.open("D:\\Wai_yan_folder\\programming\\fingerprint_recognition\\fingerprint_authentication_system\\icon\\fingerprint_icon.png")  # Replace with the actual path to your image
...
csv_button = ttk.Button(button_frame, text="Display CSV", command=click_csv_button)
csv_button.grid(row=5, column=0, pady=12)

send_email_button = ttk.Button(button_frame, text="Send Email", command=click_email_button)
